...
/* (1) Modify - Add boolean values */
/* (2) Modify - Add AND/OR values */
/* (3) Modify - Add ForDo loop values */
...
Scanner::Scanner(string s):input(s),first(0),current(0) {
  ...
  reserved["true"] = Token::TRUE; // Add (1)
  reserved["false"] = Token::FALSE; // Add (1)
  reserved["and"] = Token::AND; // Add (2)
  reserved["or"] = Token::OR; // Add (2)
  reserved["for"] = Token::FOR; // Add (3)
  reserved["endfor"] = Token::ENDFOR; // Add (3)
}
...
Token* Scanner::nextToken() {
  ...
  if (isdigit(c)) {
    ...
    case ':': token = new Token(Token::COLON); break; // Add (3)
      c = nextChar();
...
Stm* Parser::parseStatement() {
  ...
  /* Add (3) conditional for ForDo Loop */
  else if (match(Token::FOR)) {
    if (!match(Token::ID)) parserError("Se esperaba ID");
    if (!match(Token::COLON)) parserError("Se esperaba un ':' ");
    e1 = parseExp();
    if (!match(Token::COMMA)) parserError("Se esperaba una ','");
    e2 = parseExp();
    if (!match(Token::DO)) parserError("Se esperaba DO");
    b = parseBody();
    if (!match(Token::ENDFOR)) parserError("Se esperaba ENDFOR");

    s = new ForDoStatement(e1, e2, b);
  }
  /* ----- */
  else {
...
/* Add (2) - for and/or statement */
Exp* Parser::parseBExp() {
    Exp *e, *rhs;
    e = parseCExp();
    if (match(Token::AND) || match(Token::OR) ) {
        Token::Type tp = previous->type;
        BinaryOp binary_op = (tp == Token::AND) ? AND : OR;
        rhs = parseBExp();
        return new BinaryExp(e, rhs, binary_op);
    }
    return e;
}
/* ----- */
...
Exp* Parser::parseFactor() {
  ...
  /* Add (1) - Creating conditional for true/false factors */
  if (match(Token::TRUE)) {
