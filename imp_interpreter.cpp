...
/* Add (3) interpreter for ForDo Loop - variable X obligatoria */
int ImpInterpreter::visit(ForDoStatement* s) {
  auto beg = s->e1->accept(this);
  auto end = s->e2->accept(this);
  env.add_var("x", beg);
...
int ImpInterpreter::visit(BinaryExp* e) {
  ...
  case AND: result = v1 && v2; break; // Add (2)
  case OR: result = v1 || v2; break; // Add (2)
  }
  return result;
}
